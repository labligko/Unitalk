...
FROM friendlist
WHERE friend_id = $id_account AND status = 'following'";
...
<body class="overflow-x-hidden bg-gray-100 text-gray-700 font-lilita">
    <div style="margin-left: 385px; padding-top: 40px; position: relative;">
            ...
            <div class="profile-header" style="display: flex; align-items: center; gap: 20px;">
                ...
                <div class="profile-info" style="display: flex; flex-direction: column; gap: 10px;">
                    ...
                    <div style="display: flex; align-items: center; gap: 10px;">
                        ...
                        <button style="padding: 5px 15px; background-color: #74788F; color: #ffffff; border: none; border-radius: 15px; cursor: pointer;"
                            onclick="window.location.href='profile_edit.php?<?php echo $username; ?>'">Edit Profile</button>
                    </div>

                    <!-- Followers & Following -->
                    <div style="font-size: 1rem; color: #364153;">
                        <?php echo $total_follower; ?><a href="follower.php?<?php echo $username; ?>" style="text-decoration: none; color: #364153;"> Follower</a> &nbsp; • &nbsp;
                        <?php echo $total_following; ?><a href="following.php?<?php echo $username; ?>" style="text-decoration: none; color: #364153;"> Following</a>
                    </div>
                </div>
            </div>
        ...
        <p class="text-md mb-2 ml-2 mt-4 font-bold">Postingan Anda</p>

        <?php
        if (count($posts) > 0) {
            foreach ($posts as $post) {
                $id = $post['id'];
                echo "<li class='list-group-item shadow-xl' id='post-{$post['id']}' style='width: 500px; padding-bottom: 5px; margin-bottom: 10px; background-color: white; border: 1px solid #364153; color: #364153; border-radius: 15px;'>";
                ...
                echo "<a style='margin-right: 8px; font-size: 1.3rem; cursor: pointer;'onclick='likePost({$post['id']}, {$id_account})'>
                ...
                echo "<a onclick='deletePost(" . $id . ")' style='cursor: pointer; position: absolute; right: 10px; margin-right: 8px; font-size: 1.3rem;'><i class='fa-solid fa-trash'></i></a>";
                echo "</div>";
...
<script>
    ...
    function share() {
        ...
        if (!username) {
            alert("Username tidak ditemukan di URL.");
            return;
        }

        // Ganti URL menjadi search_profile
        const shareUrl = `${location.origin}/unitalk/search_profile.php?username=${encodeURIComponent(username)}`;
        console.log("URL yang disalin:", shareUrl);
